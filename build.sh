...
MEDIAPIPE_VERSION="${XRA_MEDIAPIPE_VERSION:-1.0.1}"
...
assert hasattr(vision, "HolisticLandmarker")
...
from xra_backends import downloader, engine, registry
...
# Ensure NW.js runtime is present
NW_CACHE_DIR="$ROOT_DIR/cache"
NW_VER="v0.115.0"
NW_FOLDER="nwjs-${NW_VER}-linux-x64"
NW_TAR="nwjs-${NW_VER}-linux-x64.tar.gz"
...
if [ ! -d "$NW_CACHE_DIR/$NW_FOLDER" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "ERROR: Failed to download NW.js" >&2
        exit 1
    fi
fi
