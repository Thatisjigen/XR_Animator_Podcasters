"""First-boot model check for the native MediaPipe backend.

The friend should never have to click "download". In the shipped build the
MediaPipe and the required Tasks models are bundled inside the app, so on
startup provisioning is a no-op (see :func:`autostart`): it records a
...
DEFAULT_MODEL = os.environ.get("XRA_BACKEND_MODEL", "mediapipe-tasks-landmarker")
...
def is_complete() -> bool:
    """True when both bundled Tasks models are present."""
...
def provision(force: bool = False) -> dict:
    ...
    # Development/source fallback only. Distributed builds contain both files.
...
def autostart() -> dict:
    ...
    When the default models are already bundled on disk (the shipped
